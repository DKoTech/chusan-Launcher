...
$window.Text = "CHUNITHM NEW!! - Luminous PLUS - Select aime"
...
$title.Text = " CHUNITHM NEW!! - Luminous PLUS - Select aime"
...
Get-ChildItem -Path "./Configure/aime_cards" | ForEach-Object {
    if ($_.Name -ne "selected_aime.txt") {
...
$button.Add_Click({
    # 读取这个对象，然后写入到 ./Configure/aime_cards/selected_aime.txt 里
    if ($list.SelectedItem -eq $null) {
        Write-Log "Please select aime." 2
    } else {
